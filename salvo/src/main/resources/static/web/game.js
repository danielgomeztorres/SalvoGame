		...
		dragEnter: function (ev) {
			...
			if (this.currentShip == "Vertical") {
				...
				for (var k = startLetter; k <= lastLetter; k++) { //Loop para determinar la posicion de las letras inicial en los barcos verticales
					this.saveArrayVer.push(this.letters[k] + (Number(ev.target.id.slice(1, 2))));
				}

				console.log(this.saveArrayVer);
			} else if (this.currentShip == "Horizontal") {
				this.saveArrayHor = [];
				ev.preventDefault();
				for (var i = 0; i < Number(this.child); i++) {
					this.saveArrayHor.push(ev.target.id.slice(0, 1) + (Number(ev.target.id.slice(1, 2)) + i))
					console.log(" save horizontal", this.saveArrayHor)
				}
				console.log("arrayhor", this.saveArrayHor);

			}
			console.log("final Position", this.finalPosition);
			ev.dataTransfer.setData("el", ev.target.id);
			//var currentCell = document.getElementById(ev.target.id)
			//console.log("celda del puntero :", currentCell)
		},
		// dragEnd: function (ev) {
		// 	ev.preventDefault();
		// 	//ev.dataTransfer.setData("el", ev.target.id);
